...
defobj..    lin_comb=e=
                     lambda*sum{T,sqr(sum{A,Rtilde[T,A]*x[A]})/card(T)}
                           -sum{A, Mean[A]*x[A]}   ;

tot_mass..  sum{A,x[A]} =e= 1                      ;


model optriskrwd /all/;
solve optriskrwd using nlp minimizing lin_comb;

file dem /output/;  put dem;
Put "lin_comb  = ", lin_comb.l:20:10; Put /;Put /;
Put "item         x       "; Put /;
