...
ben..     growthrate =e=
    - sum{T, log(sum{A, (x[A]*R[T,A]) } )  } / card{T};
...
solve growthopt  using nlp minimizing growthrate;


file dem /output/;  put dem;
Put "growth_rate= ", growthrate.l:20:10; Put /;          Put /;
...
Put /;
put "        Mean = ", sum{A,mean[A]*x.l[A]}:10:7,
    "    Variance = ", sum{T,sqr{sum{A, (Rtilde[T,A]*x.l[A])}} /card(T)
