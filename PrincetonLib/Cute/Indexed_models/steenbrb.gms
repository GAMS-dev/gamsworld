...
Parameter  cmd(i) ; cmd(i)  = nonz ;
Parameter  cmr(i) ; cmr(i)  = nonz ;
...
Def_obj..
 f =e= sum{i,((lt[i]*sum{j,d[j,i]}+lc[i]/cd[i]/cd[i]*power((sum{j,d[j,i]}),2))+
              (lt[i]*sum{j,r[j,i]}+lc[i]/cr[i]/cr[i]*power((sum{j,r[j,i]}),2))+
               la[i]*sqrt(cd[i]-cmd[i]+nonz) +
               la[i]*sqrt(cr[i]-cmr[i]+nonz))}/1000 ;

 d.lo[j,i] = 0.0  ;  d.l[j,i]  = 0.1 ;
 r.lo[j,i] = 0.0  ;  r.l[j,i]  = 0.1 ;
cd.lo[i] = cmd[i] ;   cd.l[i]  = 0.1 ;
cr.lo[i] = cmr[i] ;   cr.l[i]  = 0.1 ;

Model steenbrb /all/ ;

Solve steenbrb using nlp minimazing f ;
f.l=f.l*1000;
display f.l ;
