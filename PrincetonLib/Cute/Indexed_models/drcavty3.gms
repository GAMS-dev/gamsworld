...
sqr(20*y[i,j]    -8*y[i-1,j]  -8*y[i+1,j]  -8*y[i,j-1]  -8*y[i,j+1]   +
                  2*y[i-1,j+1]+2*y[i-1,j-1]+2*y[i+1,j+1]+  y[i-2,j]   +
                    y[i+2,j]  +  y[i,j-2]  + y[i,j+2]   +
(RE/4)*(y[i,j+1]-y[i,j-1])*(y[i-2,j]+y[i-1,j-1]+y[i-1,j+1]-y[i-1,j]+4*y[i+1,j]-
                            y[i+1,j-1] -y[i+1,j-1] - y[i+1,j+1]    - y[i+1,j])-
(RE/4)*(y[i+1,j]-y[i-1,j])*(y[i,j-2]+y[i-1,j-1]+y[i,j-2]-y[i,j-1]+4*y[i,j+1]-
...
Solve drcavty3 using nlp minimize f;
Solve drcavty3 using nlp minimize f;
Solve drcavty3 using nlp minimize f;
Solve drcavty3 using nlp minimize f;
Solve drcavty3 using nlp minimize f;
