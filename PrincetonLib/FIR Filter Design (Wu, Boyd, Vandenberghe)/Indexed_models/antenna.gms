...
main[j]..
R_big[j] =e=  r_small['0']
             +sum{i$inside(i),2*r_small[i]*cos((ord(i)-1)*(ord(j)-1)*step)};

Defobj..  ripple=e=alpha;
ripple_up(j)$omega_p(j)..
R_big[j] =l= sqr(alpha)/((ord(j)-1)*step)  ;

R_big.lo[j]$omega_p(j) =  1/((ord(j)-1)*step)-0.001  ;
alpha.l = 1;
...
put demo;
put "logcheb2.gms/fir_filter_design  model  output ";      put /;
