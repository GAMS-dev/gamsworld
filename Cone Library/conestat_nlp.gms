...
$echo 'loop(m, put fgen "$call gams " m.tl:0 " lp=convert rmip=convert nlp=convert optfile=1 iterlim 0 traceopt=3 trace=cone.txt "; put$xp(m) xp.te(m); put /);' >> cb.gms
...
$call cat cone.txt | cut -d, -f1,8-12,22- > modsize_nlp.txt
$eolcom #
Table trace(m,*) 
$ondelim
m,equnum,varnum,dvarnum,nz,nl_nz,stuff
$include modsize_nlp.txt
$offdelim
;
...
file ftxt / conestat_nlp.txt /
     fhtm / conestat_nlp.htm /;
...
put "Conic Models converted to NLP format using GAMS/CONVERT. Access the CONELIB" /
put "using <a href='conestat.htm'>conic constraints</a>.<P>"/ 
...
loop((mc(m,c),ms(m,s)),
  ...
  if(msol(m),
       put '<TD ALIGN=RIGHT BGCOLOR="#' c.te(c) '">' bestknown(m,'p1'):12:4
                    '</TD><TD ALIGN=RIGHT BGCOLOR="#' c.te(c) '">&nbsp</TD>'
  );
  if (not msol(m),
