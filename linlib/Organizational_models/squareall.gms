...
FILENAME: squareall.gms

PURPOSE:  Creates a solve square for LP/NLPs to compare solve/fail 
          results. User must input two tracefiles (one for each 
          solver).

          Works for LP, RMIP, NLP and MIP type models.
...
SAMPLE OUTPUT:   SQUARE:
    ...
    infeasible -       modelstat=4 or  5 or 19  solvestat=1
...
*=== Output square
put / '<html><head> <title>Solver Square Comparison - All Models</title>'
    / '<body bgcolor="#ffffff">'
...
put / 'Solver comparison utility.<br><br>'
    ...
    / 'interrupt, fail) of one solver with all return outcomes of '
...
loop(solvers,
   ...
   put$(solvernum eq 2) / 'solver ' solvers.tl:0 ' on top (columns).'
                        / 'See the <a href="#SquareDef">solver return '
                          'definitions</a> for return codes.'
);


put / '<br><br><br>'
    / '<table width="50%" border=1 >'
    / '   <tr><td align=left><b>Tracefile 1  :</b></td>'
         '<td align=right><tt>%trace1%</tt></td></tr>' 
    / '   <tr><td align=left><b>Tracefile 2  :</b></td>'
         '<td align=right><tt>%trace2%</tt></td></tr>' 
...
loop(solvers,
   ...
   if(solvernum = 1,
      put /  '   <td></td>';
      loop(result,
         put /  '   <td align=center><b> ' all_results.te(result):0 
                '  </b></td>';
      ); 
...
loop(resultA,
   put / '<tr>'
       / '   <td><b>' all_results.te(resultA):0 '</b></td>';
   loop(resultB,
     put   '   <td align=right>'
...
loop(solvers,
   solvernum = solvernum+1;
   put$(solvernum eq 2) '<tr>' / '   <td><b>total ' 
       solvers.tl:0 '</b></td>' ;
);

loop(resultB,
   if ( sum(resultA, resulttotals(resultA, resultB ) ),
      put / '   <td align=right><tt><b><a href="#solverB' 
         resultB.tl:0  '">' 
...
put / '   <td align=right><tt>' sum( (resultA,resultB), 
          resulttotals(resultA,resultB)  ):0:0 '</tt></td>' ;
put / '</tr>'
    / '</table>' //;
...
put / '<h3><a name="SquareDef">Solver return definitions:</a></h3>' 
    / '<table width="50%" border=1>';
if(is_discrete,
    put / '   <tr><td align=left><b>Outcome</b></td><td align=right>'
        ...
        / '   <tr><td align=left>integer</td><td align=right> 8 '
          '</td><td align=right> 2 or 3 or 4 </td> </tr>'
        / '   <tr><td align=left>infeasible</td><td align=right>'
          ' 4 or 10 or 19 </td><td align=right> 1 </td> </tr>'
        / '   <tr><td align=left>unbounded</td><td align=right> 3 or 18'
          ' </td><td align=right> 1 </td> </tr>'
        / '   <tr><td align=left>fail</td><td align=right> all other '
          '</td><td align=right> all other </td> </tr>'
else
    put / '   <tr><td align=left><b>Outcome</b></td><td align=right>'
          '<b>Model Status</b></td>'
          '<td align=right><b>Solver Status</b></td> </tr>'
        / '   <tr><td align=left>optimal</td><td align=right> 1 or 2'
          '</td><td align=right> 1 </td> </tr>'
        / '   <tr><td align=left>infeasible</td><td align=right>'
          '4 or 5 or 19 </td><td align=right> 1 </td> </tr>'
        / '   <tr><td align=left>unbounded</td><td align=right>'
          '3 or 18 </td><td align=right> 1 </td> </tr>'
        / '   <tr><td align=left>interrupt</td><td align=right> '
          'any </td><td align=right> 2 or 3 </td> </tr>'
        / '   <tr><td align=left>fail</td><td align=right> all other'
          '</td><td align=right> all other </td> </tr>'
);
put / '</table>' //
...
put / '<h3>Solver Resource Times</h3>'
    ...
    / 'solver resource times in seconds, as well as the ratio of' 
...
loop( (resultA,resultB),
   ...
   loop( (m(modelname),solverA,solverB),          
      if ( mstat(modelname,solverA,resultA) and 
                mstat(modelname,solverB,resultB),
         if(doheader,
            put / '<a name="' resultA.tl:0 resultB.tl:0 '">'; 
            put / '<h4>'solverA.tl:15 ': '
            ...
            put /  '   <td align=left><b>Modelname</b></td>'                
                /  '   <td align=right><b>' solverA.tl:>15 '</b></td>'
                /  '   <td align=right><b>' solverB.tl:>15 '</b></td>'
                /  '   <td align=center><b>Ratio ('solverA.tl:0 '/' 
                        solverB.tl:0 ') </b></td>'
                   
            if( not sameas(resultA,"opt"),
               put /  '   <td align=right><b>Status ('solverA.tl:0
                      ') </b></td>';
            else
               put /  '   <td align=right><b>Obj ('solverA.tl:0
                      ') </b></td>';
            );
                   
            if( not sameas(resultB,"opt"),
               put /  '   <td align=right><b>Status ('solverB.tl:0
                      ') </b></td>';
            else
               put /  '   <td align=right><b>Obj ('solverB.tl:0
                      ') </b></td>';
            );
            put    /  '</tr>';
         ...
         put / '<tr>'
             / '   <td align=left><tt>' modelname.tl:15 
               '</tt></td>';
         put / '   <td align=right><tt>' resA(modelname):>15:4 
               '</tt></td>' 
             / '   <td align=right><tt>' resB(modelname):>15:4 
               '</tt></td>';
...
*           === Output objective values (minimizing)
         ...
         elseif( sameas(resultA,"opt")),                
            put / '   <td align=right>---</td>'
                / '   <td align=right><tt>' objA(modelname):0:8 
                  '</tt></td>'
                / '   <td align=right><tt> mstat(' modstatB(modelname):2:0 
                  ') sstat(' solstatB(modelname):2:0  ')</tt></td>';  
         elseif( sameas(resultB,"opt")),                
            put / '   <td align=right>---</td>'
                / '   <td align=right><tt> mstat(' modstatA(modelname):2:0 
                  ') sstat(' solstatA(modelname):2:0  ')</tt></td>'   
                / '   <td align=right><tt>' objB(modelname):0:8 '</tt></td>';
         else
            put / '   <td align=right>---</td>'
                / '   <td align=right><tt> mstat(' modstatA(modelname):2:0 
                  ') sstat(' solstatA(modelname):2:0  ')</tt></td>' 
      ...
      if ( mstat(modelname,solverA,resultA),
         if(doheader,
            ...
            put / '<h4>'solverA.tl:10 ': '
                  resultA.te(resultA):>10 ': &nbsp&nbsp&nbsp&nbsp'
      ...
      if ( mstat(modelname,solverB,resultB),
         if(doheader,
            ...
            put / '<h4>'solverB.tl:10 ': '
                  resultA.te(resultB):>10 ': &nbsp&nbsp&nbsp&nbsp'
                  '<a href="#top">Back to top</a></h4>'/;
         ...
         if( sameas(resultB,"opt"),   
            put / '   <td align=right><tt>' objA(modelname):0:8 '</tt></td>';
         else
            put / '   <td align=right><tt>mstat(' modstatB(modelname):2:0 
                  ') sstat(' solstatB(modelname):2:0  ')</tt></td>';
         );
         put / '</tr>';
      );
