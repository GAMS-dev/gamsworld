...
PURPOSE:  Creates two tables of comparative solve resource times of two 
          ...
          For the first table, optimal is defined as modelstat=1 or 2, 
          solvestat=1. For the second, if one solver found a solution, 
          but the other did not, the first solver is considered infinitely 
          faster. If both failed to find a solution, then the resource times 
          are considered the same.

          Also, each resource time category (same, faster, much faster, etc.
          is subdivided into subcategories, comparing objective function
          values.  
          Output is in .txt or in .html files. Also lists models for
          each resource time category.

          Works for NLP type models only.
...
Parameter restimes(restype)              "resource times by category", 
          modelrestime(*,restype)        "resource times by model and category",
          modelresobj(*,restype,objtype) "resource times by model, resource and obj category",
...
*=== Disaggregate resource time categories by objective function categories
...
Parameter nummodels /0/;


if (list_text("%text%") = list_text("yes"),

   fresA.nr = 2;
   put fresA 'NLP Table A:  Considers only models solved optimally by both solvers. ' //
   ...
   put      'The objective value of one solver is considered better than that of the' /
   put      'other if the relative objective difference is greater than ' bound:0 '.' //;    
   ...
   solvernum = 0;
   put ' ':>45;
   loop(objtype, put objtype.tl:>10 );

   loop(solvers,
      solvernum = solvernum+1;
      if(solvernum = 1,  
         put /   'Solver ' solvers.tl:8 ' much faster       : '
                           restimes("Amfaster"):>6:0;
         loop(objtype, 
            nummodels =  sum(modelname, modelresobj(modelname,"Amfaster",objtype));
            put nummodels:>10:0;
         );
 
         put /   'Solver ' solvers.tl:8 ' faster            : '
                           restimes("Afaster"):>6:0 ;
         loop(objtype, 
            nummodels =  sum(modelname, modelresobj(modelname,"Afaster",objtype));
            put nummodels:>10:0;
         );
         put /   'Solvers perform the same':33            ' : '
                           restimes("same"):>6:0 ;
         loop(objtype, 
      ...
      elseif(solvernum = 2),
         ...
         loop(objtype, 
            nummodels =  sum(modelname, modelresobj(modelname,"Bfaster",objtype));
            put nummodels:>10:0;
         );
         put /   'Solver ' solvers.tl:8 ' much faster       : '
                           restimes("Bmfaster"):>6:0 ;
         loop(objtype, 
            nummodels =  sum(modelname, modelresobj(modelname,"Bmfaster",objtype));
            put nummodels:>10:0;
         );
      );
   );
   ...
   loop(objtype, 
      nummodels =  sum( (modelname,restype), modelresobj(modelname,restype,objtype));
      put nummodels:>10:0;
   );  
   put //; 
   ...
   loop((restype,objtype),
      ...
      if(sum(modelname,modelrestime(modelname,restype)),
         ...
         if ( sameas(restype,"Amfaster"),
            if ( sameas(objtype, "ObjAbet")$sum(modelname, modelresobj(modelname,restype,objtype)  ),
               put // 'Solver ';
               loop(solverA, put solverA.tl:0);
               put ' much faster - ' 
               put 'Objective of solver ';
               loop(solverA, put solverA.tl:0);
               put ' better:' ;
               ...
               loop(solverA, put $75 'Obj ':>10 solverA.tl:15 );
               loop(solverB, put @95 'Obj ':>10 solverB.tl:15 /);
               put '----------------------------------------------------------------------------------------------------------------' 
            elseif( sameas(objtype,"Objsame")$sum(modelname, modelresobj(modelname,restype,objtype)) ),
               put // 'Solver ';
               loop(solverA, put solverA.tl:0);
               put ' much faster - ' 
               put 'Objective of solvers are the same: '
               ...
               loop(solverB, put @95 'Obj ':>10 solverB.tl:15 /);
               put '----------------------------------------------------------------------------------------------------------------' 
            elseif( sameas(objtype,"ObjBbet")$sum(modelname, modelresobj(modelname,restype,objtype)) ),
               put // 'Solver ';
               loop(solverA, put solverA.tl:0);
               put ' much faster - ' 
               put 'Objective of solver ';
            ...
            put //;
         elseif( sameas(restype,"Afaster")$sum(modelname, modelresobj(modelname,restype,objtype)) ),
            ...
            put //;
         elseif( sameas(restype,"same")$sum(modelname, modelresobj(modelname,restype,objtype)) ),
            if ( sameas(objtype, "ObjAbet"),
               put // 'Solver perform the same - ';
               put 'Objective of solver ';
               loop(solverA, put solverA.tl:0);
               put ' better:' ;
            ...
            put //;
         elseif( sameas(restype,"Bfaster")$sum(modelname, modelresobj(modelname,restype,objtype)) ),
            if ( sameas(objtype, "ObjAbet"),
               put // 'Solver ';
               loop(solverB, put solverB.tl:0);
               put ' faster - ' 
            ...
            put //;
         elseif( sameas(restype,"Bmfaster")$sum(modelname, modelresobj(modelname,restype,objtype)) ),
            if ( sameas(objtype, "ObjAbet"),
               put // 'Solver ';
               loop(solverB, put solverB.tl:0);
               put ' much faster - ' 
      ...
      loop(modelname$modelresobj(modelname,restype,objtype),
         ...
         put ObjA(modelname):>25:8 ObjB(modelname):>20:8 /;
...
if (list_html("%html%") eq list_html("yes"),
   ...
   put / '<html> <head> <title>NLP Resource Time Comparison - Optimally Solved Models</title>' 
       / '<body bgcolor="#ffffff">';
    
   fresA_htm.nr = 2;

   put   '<h2>NLP Table A:&nbsp&nbsp  Considers only models solved optimally by both solvers. </h2>' /
       / 'Total resource time for models solved optimally by solver A and solver B.<br><br>'
   if(bound = inf,
      put    / 'Optimal solve is defined as modelstat=1, solvestat=1. Solutions are not'/
      put      'checked for how close they are to eachother.' //;
   else
      put    / 'Optimal solve is defined as modelstat=1 or 2 and solvestat=1, Solutions'
      put    / 'are not checked for how close they are to oneanother.';
   );

   fresA_htm.nr = 1;

   put / 'Resource times are considered the same, if they are within ' %tsame%:0:0 '%' /
         'of eachother. A solver is considered faster than another, if it is less ' /
         'than ' %tfaster%:0:0 '% faster than the other. A solver is considered much' /
         'faster than another, if it is more than ' %tfaster%:0:0 '% faster than the ' /
         'other.' //;
   ...
   put / '<br><br>'
       ...
       / 'relative objective value difference is greater than ' bound:0:2 '.'  //; 
   ...
   loop(solvers,
     ...
     if((solvernum eq 1),
          ...
          if (restimes("Amfaster"),
              put '<a href="#Amfaster"><tt>' restimes("Amfaster"):>6:0 '</tt></a></td>';
          else
              put '- </td>';
          );
          loop(objtype,
     ...
     if((solvernum eq 1),
          put  /  '<tr>'
          put  /  '   <td align=left>Solver ' solvers.tl:8 ' faster       :</td>';
          put  /  '   <td align=right>';
          if (restimes("Afaster"),
              put '<a href="#Afaster"><tt>' restimes("Afaster"):>6:0 '</tt></a></td>';
          else
              put '- </td>';
          );
          loop(objtype,
             nummodels =  sum(modelname, modelresobj(modelname,"Afaster",objtype));
             put$nummodels  / '   <td align=right><a href=#Afaster' objtype.tl:0 '><tt>'  nummodels:>10:0 '</tt></a></td>';
             put$(not nummodels) / '   <td align=right> - </td>'; 
          );
          put / '</tr>';
     );
     if((solvernum eq 1),
          ...
          loop(objtype,
             nummodels =  sum(modelname, modelresobj(modelname,"same",objtype));
             put$nummodels / '   <td align=right><a href=#same' objtype.tl:0 '><tt>'  nummodels:>10:0 '</tt></a></td>';
             put$(not nummodels) / '   <td align=right> - </td>'; 
          );
          put / '</tr>';
     );
     if((solvernum eq 2),
          ...
          put  /  '   <td align=right>'
          if (restimes("Bfaster"),
              put '<a href="#Bfaster"><tt>' restimes("Bfaster"):>6:0 '</tt></a></td>';
          else
              put '- </td>';
          );
          loop(objtype,
             nummodels =  sum(modelname, modelresobj(modelname,"Bfaster",objtype));
             put$nummodels / '   <td align=right><a href=#Bfaster' objtype.tl:0 '><tt>'  nummodels:>10:0 '</tt></a></td>';
             put$(not nummodels) / '   <td align=right> - </td>'; 
          );
          put / '</tr>';
     );
     if((solvernum eq 2),
          ...
          if (restimes("Bmfaster"),
              put '<a href="#Bmfaster"><tt>' restimes("Bmfaster"):>6:0 '</tt></a></td>';
          else
              put '- </td>';
          );
          loop(objtype,
             nummodels =  sum(modelname, modelresobj(modelname,"Bmfaster",objtype));
             put$nummodels / '   <td align=right><a href=#Bmfaster' objtype.tl:0 '><tt>'  nummodels:>10:0 '</tt></a></td>';
             put$(not nummodels) / '   <td align=right> - </td>'; 
          );
          put / '</tr>';
     );
   ...
   loop(objtype,
      nummodels = sum( (modelname,restype), modelresobj(modelname,restype,objtype) );
      put$nummodels / '   <td align=right><b><tt> ' nummodels:>10:0 '</tt></b></td>';
      put$(not nummodels) / '   <td align=right> - </td>'; 
   );   
   put / '</tr>';
   put / '</table>';
   ...
   loop((restype,objtype),
      ...
      if(sum(modelname,modelresobj(modelname,restype,objtype)),
         ...
         loop(solverB, put solverB.tl:0 ')' );
         put   '</b></td>';
         loop(solverA, put '<td align=right><b>Obj ' solverA.tl:0 '</b></td>');
         loop(solverB, put '<td align=right><b>Obj ' solverB.tl:0 '</b></td>');
         put   /  '</tr>' ;
      );
      ...
      loop(modelname$modelresobj(modelname,restype,objtype),
         ...
         if (sameas(objtype,"ObjAbet"),
            put / '   <td align=right><b><tt>' ObjA(modelname):>25:8 '</tt></b></td><td align=right><tt>' ObjB(modelname):>20:8 '</tt></td>' /;
         elseif(sameas(objtype,"ObjBbet")),
            put / '   <td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><b><tt>' ObjB(modelname):>20:8 '</tt></b></td>' /;
         else
            put / '   <td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><tt>' ObjB(modelname):>20:8 '</tt></td>' /;
         );
...
if(list_text("%text%") eq list_text("yes"),
   ...
   fresB.nr = 2;
   put fresB 'NLP Table B:  Considers all models. ' //
   put      'Total resource time for all models. Optimal solve is defined as ' /
   put      'modelstat=1 or 2 and solvestat=1. Objective values are not checked' / 
   put      'for how close they are to oneanother.' //;
   ...
   put      'The objective of one solver is considered better than the other, if the' /
   put      'relative objective difference is greater than ' bound:0 '.' // ;  
   ...
   loop(solvers,
     solvernum = solvernum+1;
     if(solvernum = 1,  
           put /  'Solver ' solvers.tl:8 ' infinitely faster      : '
                                    restimes("Aifaster"):>6:0 ;
           loop(objtype,
               nummodels = sum(modelname, modelresobj(modelname,"Aifaster",objtype) );
               put nummodels:>10:0;
           );
           put /  'Solver ' solvers.tl:8 ' much faster            : '
                                    restimes("Amfaster"):>6:0 ;
     ...
     elseif(solvernum = 2),
           ...
           loop(objtype,
               nummodels = sum(modelname, modelresobj(modelname,"Bifaster",objtype) );
               put nummodels:>10:0;
           );
           put /  'Both solvers failed to solve optimally : '
                                    restimes("ABnonopt"):>6:0 ;
           loop(objtype,
               nummodels = sum(modelname, modelresobj(modelname,"ABnonopt",objtype) );
               put nummodels:>10:0;
           );
     );
   );
   ...
   loop((restype,objtype),
      ...
      loop(modelname$modelresobj(modelname,restype,objtype),
         ...
         if(bothsolve(modelname),
            ...
            fresB.nr = 2;
            put objA(modelname):>28:8  objB(modelname):>18:8;
            fresB.nr = 1;
...
if (list_html("%html%") eq list_html("yes"),
   ...
   put / '<html> <head> <title>NLP Resource Time Comparison - All Models</title>' 
       / '<body bgcolor="#ffffff">'
     
   put / '<h2>NLP Table B:&nbsp&nbsp  Considers all models. </h2>' /
       / 'Total resource time for all models.<br><br>' //
   put      'Optimal solve is defined as modelstat=1 or 2 and solvestat=1. Solutions are not'/
   put      'checked for how close they are to eachother.' /;
   ...
   loop(solvers,
     ...
     if((solvernum eq 1),
         put  /  '<tr>'
              /  '   <td align=left>Solver ' solvers.tl:8 ' infinitely faster :</td><td align=right>'
         if(restimes("Aifaster"),
            put  '<a href="#Aifaster"><tt>' restimes("Aifaster"):>6:0 '</tt></a></td>';
         else
            put  '-</td>';
         );
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"Aifaster",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#Aifaster' objtype.tl:0  '"><tt>' nummodels:0:0 '</tt></a></td>';
            else
     ...
     if((solvernum eq 1),
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"Amfaster",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#Amfaster' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
     ...
     if((solvernum eq 1),
         put  /  '<tr>'
              /  '   <td align=left>Solver ' solvers.tl:8 ' faster :</td><td align=right>';
         if(restimes("Afaster"),
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"Afaster",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#Afaster' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
     ...
     if((solvernum eq 1),
         put  /  '<tr>'
              /  '   <td align=left>Solvers perform the same':0             ' :</td><td align=right>';
         if(restimes("same"),
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"same",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#same' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
     ...
     if((solvernum eq 2),
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"Bfaster",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#Bfaster' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
     ...
     if((solvernum eq 2),
         put  /  '<tr>'
              /  '   <td align=left>Solver ' solvers.tl:8 ' much faster :</td><td align=right>';
         if(restimes("Bmfaster"),
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"Bmfaster",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#Bmfaster' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
     ...
     if((solvernum eq 2),
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"Bifaster",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#Bifaster' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
     ...
     if((solvernum eq 2),
         put  /  '<tr>'
              /  '   <td align=left>Both solvers failed to solve optimally :</td><td align=right>';
         ...
         loop(objtype,
            nummodels = sum(modelname, modelresobj(modelname,"ABnonopt",objtype) );
            if (nummodels,
               put  / '   <td align=right><a href="#ABnonopt' objtype.tl:0 '"><tt>'  nummodels:0:0 '</tt></a></td>';
            else
               put  / '   <td align=right> - </td>';
            );
         );
   ...
   loop( (restype,objtype),
      ...
      if(sum(modelname,modelresobj(modelname,restype,objtype)),
         ...
         elseif( sameas(restype,"same")),
            put // '<a name="same"></a>';
            put /  '<h4><a name="same' objtype.tl:0  '">Solvers perform the same:'
                         '</a>&nbsp&nbsp&nbsp&nbsp<a href="#top">Back to top</a></h4>' /;
         elseif( sameas(restype,"Bfaster")),
         ...
         elseif( sameas(restype,"ABnonopt")),
            ...
            put /  '<h4><a name="ABnonopt' objtype.tl:0 '">Both solvers failed to solve optimally - ';
            if (sameas(objtype,"ObjAbet"),
                put 'Obj of ';
                loop(solverA, put solverA.tl:0 );
                put ' better:';
      ...
      loop(modelname$modelresobj(modelname,restype,objtype),
         ...
         elseif(sameas(restype,"Aifaster")),
            put / '<tr><td align=left><tt>' modelname.tl:15 '</tt></td>'
                '<td align=right><tt>' resA(modelname):0:4 '</tt></td><td align=right>'; 
                loop( (solverB,result),
                   if( square(modelname,solverB,result),
                      put result.te(result):0;
                   );
                );  
            put '</td><td align=center> -- </td>' ;
            if (sameas(objtype,"ObjAbet"),
               put / '   <td align=right><b><tt>' ObjA(modelname):>25:8 '</tt></b></td><td align=right></tt>' ObjB(modelname):>20:8 '</tt></td>' /;
            elseif(sameas(objtype,"ObjBbet")),
               put / '   <td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><b><tt>' ObjB(modelname):>20:8 '</tt></b></td>' /;
         ...
         elseif(sameas(restype,"Bifaster")),
            put / '<tr><td align=left><tt>' modelname.tl:15 '</tt></td><td align=right>';
                loop( (solverA,result),
                   if( square(modelname,solverA,result),
                      put result.te(result):0;
                   );
                );
            put '</td><td align=right><tt>' resB(modelname):15:4 '</tt></td><td align=center> -- </td>' ;
            if (sameas(objtype,"ObjAbet"),
               put / '   <td align=right><b><tt>' ObjA(modelname):>25:8 '</tt></b></td><td align=right><tt>' ObjB(modelname):>20:8 '</tt></td>' /;
            elseif(sameas(objtype,"ObjBbet")),
               put / '   <td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><b><tt>' ObjB(modelname):>20:8 '</tt></b></td>' /;
         ...
         elseif(nosolve(modelname)),
            ...
            put   '</td><td align=center> -- </td>' ;
            put '<td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><tt>' ObjB(modelname):>20:8 '</tt></td></tr>' /;
         );
      );
