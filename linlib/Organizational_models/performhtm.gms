...
*
*  models by credits (author) htm file
*
...
put fcred '<html> <head> <title>LINLib Credits</title>'
       /  '<!--#include virtual="/performance/header.html" -->'
...
put       / 'We have made every attempt to give credit to the original author'
          ...
          / 'Performance World coordinator at'
          / '<a href="mailto:apruessner@gams.com">apruessner@gams.com</a>'
          / '<br><br>'
          / '<hr>'

put    /  '<h2>LINLib Credits:</h2>'
       / '<table width="100%">';
scalar cnter /0/;
...
*
*  models by size classification htm file
*
...
put fsiz '<html> <head> <title>LINLib Models by Size</title>'
       /  '<!--#include virtual="/performance/header.html" -->'
put fsiz   '<h2>Remarks:</h2>'
put       / 'We have made every attempt to give credit to the original author'
          / 'or authors of the individual models. Unfortunately, since some models'
          / 'come from large model collections or are derived from client models,'
          / 'the correct reference is not always available. <br><br>'
...
put    /  '<h2>LINLib Models by size:</h2>'
       / '<table width="100%">';

cnter = 0;
loop(modsize,
  put / '<tr>'
  put / '<td align=left><a href="#' modsize.tl:0 '">'modsize.te(modsize):0 '</a></td>' 
  put$(not sameas(modsize,"huge"))   '<td align=left>(Number of nonzeros between ' size_bounds(modsize,"lo"):0:0 ' and ' 
                                     size_bounds(modsize,"hi"):0:0 ')';
  put$(sameas(modsize,"huge"))   '<td align=left>(Number of nonzeros greater than ' size_bounds(modsize,"lo"):0:0 ')';
  put   '</td>'
);
...
Parameter nonzeros /0/;

loop(modsize,
  put / '<a name="' modsize.tl:0 '"></a>'
      / '<h4>' modsize.te(modsize) ':</h4>';
  put / '<ul>';

  put / '<li> Models: ';
  put$(not sameas(modsize,"huge"))   '<td align=left>(Number of nonzeros between ' 
                                     size_bounds(modsize,"lo"):0:0 ' and ' 
                                     size_bounds(modsize,"hi"):0:0 ')';
  put$(sameas(modsize,"huge"))   '<td align=left>(Number of nonzeros greater than ' 
                                   size_bounds(modsize,"lo"):0:0 ')';
  put / '<ul>';

  loop(m,
     nonzeros = sum(mtype, stats(m,mtype,"NZ") ); 
     if( size_bounds(modsize,"lo")<nonzeros  and  nonzeros<=size_bounds(modsize,"hi") ,
        put / '<li><a href="' m.tl:0 '.htm">' m.tl:0 '</a>&nbsp'
     )
  );
...
*
*  models by origin classification htm file
*

file fori / origin.htm /;
fsiz.pw=1000;


put fori '<html> <head> <title>LINLib Models by Origin</title>'
       /  '<!--#include virtual="/performance/header.html" -->'
put fori   '<h2>Remarks:</h2>'
put       / 'We have made every attempt to give credit to the original author'
          / 'or authors of the individual models. Unfortunately, since some models'
          / 'come from large model collections or are derived from client models,'
          / 'the correct reference is not always available. <br><br>'
...
put    /  '<h2>LINLib Models by origin:</h2>'
       /  'The models come from the following repositories and collections:<br><br>'
       /  '<table width="100%">';


loop(d,
   put / '<tr>'
   put / '<td align=left><a href="#' d.tl:0 '">'d.tl:0 '</a></td>' 
   put   '</td>'
);
...
loop(d,
  ...
  runmodels(m) = sum( (s,d_alias)$(ds(d_alias,s) and sameas(d,d_alias) and ms(m,s)), ord(m) );

  put / '<h4><a name="' d.tl:0  '">' d.tl:20 '</a>:</h4>';
  put / '<ul>';

  put / '<li> ' d.te(d):0;
  loop(dr(d,r), put / '<li>' r.te(r));
  put / '<li> Models: ';
  put / '<ul>';
...
put fpubs  '<html> <head> <title>LINLib Bibliography</title>'
       / '<!--#include virtual="/performance/header.html" -->'
       / '<h2>LINLib Bibliography</h2>';
