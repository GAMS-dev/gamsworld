...
*=== Output models 
   ...
   put / 'Models for each solver pair outcome. Listed are the GAMS model and' 
       / 'solve statuses.' //
      ...
      loop( (m(modelname),solverA,solverB),      
            ...
            if ( mstat(modelname,solverA,resultA) and mstat(modelname,solverB,resultB),
               ...
               if(doheader,
                  ...
                  put 'Modelname':15                
                  put solverA.tl:>8 ' mstat':0;
                  put solverA.tl:>8 ' sstat':0;
                  put solverB.tl:>8 ' mstat':0;
                  put solverB.tl:>8 ' sstat':0 / ;
               ...
               put modelname.tl:15;
               put mstat(modelname,solverA,resultA):>14:0  sstat(modelname,solverA,resultA):>14:0  
                   mstat(modelname,solverB,resultB):>14:0  sstat(modelname,solverB,resultB):>14:0 /;  
            );
      );
...
*=== Output square
   ...
   put / '<br><br><br>'
       ...
       / '   <tr><td align=left><b>Tracefile 1  :</b></td><td align=right>%trace1%</td></tr>' 
       / '   <tr><td align=left><b>Tracefile 2  :</b></td><td align=right>%trace2%</td></tr>' 
       / '   <tr><td align=left><b>Solvers used :</b></td> ';

   solvernum = 0;
   loop(solvers,
      solvernum = solvernum+1;
      put$(solvernum eq 1)   '<td align=right>' solvers.tl:0 '</td></tr>';
      put$(solvernum eq 2) / '   <tr><td></td><td align=right>' solvers.tl:0 '</td></tr>' /;
   );
   put / '</table><br>';
   ...
   loop(solvers,
      ...
      put$(solvernum eq 1)  /  '   <td></td>'
                            ...
                            /  '   <td align=center><b>interrupt </b></td>'
   ...
   loop(resultA,
      ...
      loop(resultB,
        ...
        if(resulttotals(resultA,resultB),
           put  '<b><a href="#' resultA.tl:0 resultB.tl:0   '">' resulttotals(resultA, resultB):0:0 '</b></a>';
        else
           put  '-';
        );
        put   '</td>' /;
      );
      put   '   <td align=right>' sum(resultB, resulttotals(resultA, resultB ) ):0:0 '</td>';
      put / '</tr>';
   );
   ...
   loop(resultB,
      put / '   <td align=right>' sum(resultA, resulttotals(resultA, resultB ) ):0:0 '</td>' ;
   );
   put / '   <td align=right>' sum( (resultA,resultB), resulttotals(resultA,resultB)  ):0:0 '</td>' ;
...
*=== Output models 
   ...
   put / '<h3>Model and Solver Statuses</h3>'
       / 'Models for each solver pair outcome. Listed are the GAMS model and' 
       / 'solve statuses.<br><br>' //
            ...
            if ( mstat(modelname,solverA,resultA) and mstat(modelname,solverB,resultB),
               ...
               if(doheader,
                  ...
                  put / '<h4>'solverA.tl:10 ': '
                         resultA.te(resultA):>10 '&nbsp -- &nbsp';
                  put    solverB.tl:10 ': '
                  ...
                  put   '<td align=left><b>Modelname</b></td>'                
                        '<td align=right><b>' solverA.tl:>8 ' mstat':0 '</b></td>'
                        '<td align=right><b>' solverA.tl:>8 ' sstat':0 '</b></td>'
                        '<td align=right><b>' solverB.tl:>8 ' mstat':0 '</b></td>'
                        '<td align=right><b>' solverB.tl:>8 ' sstat':0 '</b></td>'
                        '</tr>';
               ...
               put / '<tr>'
                     '<td align=left>' modelname.tl:15 '</td>';
               put   '<td align=right>' mstat(modelname,solverA,resultA):>2:0 '</td>' 
                     '<td align=right>' sstat(modelname,solverA,resultA):>2:0 '</td>'  
                     '<td align=right>' mstat(modelname,solverB,resultB):>2:0 '</td>' 
                     '<td align=right>' sstat(modelname,solverB,resultB):>2:0 '</td>'
                     '</tr>' ;  
            );
      );
