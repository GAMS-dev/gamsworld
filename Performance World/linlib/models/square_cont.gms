...
PURPOSE:  Creates a solve square for LP/NLPs to compare solve/fail results.
          User must input two tracefiles (one for each solver).

          Works for LP and NLP type models (no discrete variables allowed)
...
loop(resultA,
   ...
   if ( sum(resultB, resulttotals(resultA, resultB ) ),
      put   '   <td align=right><tt><b><a href="#solverA' resultA.tl:0  '">'
             sum(resultB, resulttotals(resultA, resultB ) ):0:0 '</a></b></tt></td>';
   else
...
loop(solvers,
   solvernum = solvernum+1;
   put$(solvernum eq 2) '<tr>' / '   <td><b>total ' solvers.tl:0 '</b></td>' ;
);

loop(resultB,
   if ( sum(resultA, resulttotals(resultA, resultB ) ),
      put / '   <td align=right><tt><b><a href="#solverB' resultB.tl:0  '">' 
...
put / '<h3><a name="#SquareDef">Solver return definitions:</a></h3>' 
    ...
    / '   <tr><td align=left>infeasible</td><td align=right> 4 or 5 or 19 </td><td align=right> 1 </td> </tr>'
...
put / '<li>Solver resource time ratios for a particular model are listed only '
      'if one solver has resource greater than ' resourcemin:0  '.';
...
*           === Output objective values (minimizing)
         ...
         elseif( sameas(resultA,"opt")),                
            put / '   <td align=right>---</td>'
                ...
                / '   <td align=right><tt> mstat(' modstatB(modelname):2:0 ') sstat(' solstatB(modelname):2:0  ')</tt></td>';  
         elseif( sameas(resultB,"opt")),                
            put / '   <td align=right>---</td>'
                / '   <td align=right><tt> mstat(' modstatA(modelname):2:0 ') sstat(' solstatA(modelname):2:0  ')</tt></td>'   
      ...
      if ( mstat(modelname,solverA,resultA),
         if(doheader,
            ...
            put / '<h4>'solverA.tl:10 ': '
                  resultA.te(resultA):>10 ': &nbsp&nbsp&nbsp&nbsp<a href="#top">Back to top</a></h4>'/;
      ...
      if ( mstat(modelname,solverB,resultB),
         if(doheader,
            ...
            put / '<h4>'solverB.tl:10 ': '
                  resultA.te(resultB):>10 ': &nbsp&nbsp&nbsp&nbsp<a href="#top">Back to top</a></h4>'/;
