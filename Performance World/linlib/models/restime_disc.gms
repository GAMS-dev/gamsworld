...
PURPOSE:  Creates two tables of comparative solve resource times of two 
          ...
          values.  
          Output is in .html files. Also lists models for each resource 
          time category.

          Works for MIP modeltypes.
...
OPTIONAL INPUTS:
          ...
          --outfile: root filename of restimes output file (default is
                     ...
                     out_resB.htm)
          --resmin:  minimum resource time threshold. If both solvers report
                     a resource time below --resmin, then no ratio is reported.
                     In this case, both solver times are considered the same.
                     (default 0.05).
...
Parameter bound        "relative objc func difference threshold" / %bnd% /
          resourcemin  "min resource time threshold for ratio"   / %resmin%/;
...
loop( (modelname,modeltype,solvername,nlpdef,mipdef)
    ...
    elseif( modstat=8 and (solstat=2 or solstat=3 or solstat=4) ),
       square(modelname,solvername,"int") = yes;
...
put / '<html> <head> <title>Resource Time Comparison - Optimally Solved Models</title>' 
    / '<body bgcolor="#ffffff">';
 
fresA_htm.nr = 2;

put   '<h2>Table A:&nbsp&nbsp  Considers only models solved optimally by both solvers. </h2>' /
...
put / '<ul>'
    / '<li>Models for each resource time category. Listed are the solver resource times in'
    / ' seconds, as well as the ratio of resource times for the two solvers if both solved optimally.' 
    / '<li>Also listed are the objective values using both solvers. The better solution found is listed'
    / 'in boldface. A solution is considered better, if the relative objective function difference is'
    / ' greater than ' bound:0 '.'  
    / '<li>Solver resource time ratios for a particular model are listed only if both solvers are optimal '
...
put / '<ul>'
    ...
    / 'in boldface. A solution is considered better, if the relative objective function difference is'
    / ' greater than ' bound:0 '.'  
    / '<li>Solver resource time ratios for a particular model are listed only if both solvers are '
    / 'optimal and if one solver has resource time greater than ' resourcemin '.' 
    / '</ul>' //;
...
loop( (restype,objtype),
   ...
   loop(modelname$modelresobj(modelname,restype,objtype),
      ...
      if(bothsolve(modelname),
         ...
         fresB_htm.nr = 2;
         if (sameas(objtype,"ObjAbet"),
            put / '   <td align=right><b><tt>' ObjA(modelname):>25:8 '</tt></b></td><td align=right><tt>' 
                       ObjB(modelname):>20:8 '</tt></td>' /;
         elseif(sameas(objtype,"ObjBbet")),
            put / '   <td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><b><tt>' 
                       ObjB(modelname):>20:8 '</tt></b></td>' /;
         else
            put / '   <td align=right><tt>' ObjA(modelname):>25:8 '</tt></td><td align=right><tt>' 
                       ObjB(modelname):>20:8 '</tt></td>' /;
         );
         fresB_htm.nr = 1;
         put '</tr>'
      elseif(sameas(restype,"Aifaster")),
         put / '<tr><td align=left><tt>' modelname.tl:15 '</tt></td>'
             '<td align=right><tt>' resA(modelname):0:4 '</tt></td><td align=right><tt>'; 
             loop( (solverB,result),
      ...
      elseif(sameas(restype,"Bifaster")),
         ...
         put '</tt></td><td align=right><tt>' resB(modelname):15:4 '</tt></td><td align=center> -- </td>';
         fresB_htm.nr = 2;
         if (sameas(objtype,"ObjAbet"),
            put / '   <td align=right><b><tt>' ObjA(modelname):>25:8 '</tt></b></td><td align=right><tt>' 
                   ObjB(modelname):>20:8 '</tt></td>' /;
