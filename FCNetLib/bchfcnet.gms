...
* If you need to verify a solution written by savepoint add --loadpoint=1to the command line
$if not set loadpoint $set loadpoint 0
$set mipsolver %gams.mip%
$if x%mipsolver%==x $set mipsolver cplex
$ifThen not loadpoint==0
$echo mipstart 1 >> cplex.opt
$echo mipstart 1 > %mipsolver%.op2
$if %gams.optfile%==0 master.optfile=2;
execute_loadpoint "points%system.dirsep%%fname%.gdx", x, y;
$endif
