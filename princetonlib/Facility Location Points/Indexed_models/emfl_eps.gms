...
Set i1(i) /i1*i5/;
Alias(i1,i2);
parameter epsilon ; epsilon =1.0e-8 ;
parameter  a[k,j] ;
...
sumEucl=e=
   sum{k,sum(i,                  (w[k,i] *sqrt(epsilon + sum{j,sqr(x[i,j]-a[k,j] )} )))}
 + sum{i,sum(ii$(ord(ii)>ord(i)),(v[i,ii]*sqrt(epsilon + sum{j,sqr(x[i,j]-x[ii,j])} )))}
                   ;
...
solve emfl_eps using nlp minimaze sumEucl;
