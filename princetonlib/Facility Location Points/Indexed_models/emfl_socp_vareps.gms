...
Def_s(k,i)..
 (s(k,i))  =g= sqrt(epsilon*epsilon+sum{j,sqr(x[i,j]-a[k,j] )})    ;

def_t(i,ii)..
 (t(i,ii)) =g= sqrt(epsilon*epsilon+sum{j,sqr(x[i,j]-x[ii,j])})    ;
...
model emfl_scp_vrps /all/;

For (mn = 1 to 1,

solve emfl_scp_vrps using nlp minimaze sumEucl;
