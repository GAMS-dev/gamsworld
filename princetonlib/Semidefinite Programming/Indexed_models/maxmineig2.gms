...
Parameter b(k); b(k) = 0; b('k%m1%') = 1 ;

Loop(k$(ord(k) ne card(k)),
Loop(i,
Loop(j,A[j,j,k] = -1/%n1%);

       A[i,i,k]$(ord(i) eq ord(k) ) = 1-1/%n1%;
...
Loop(i,
Loop(j$(ord(j) ge ord(i)),
  C[i,j] = -C[i,j];
    );
    );

*#############################################################
*#  Min-max eigenvalue as described in HRVW
*#
*
*param eps := 1.0e-8;
*
...
Model maxmineig2 /all/;
Solve maxmineig2 using nlp minimazing cost;
