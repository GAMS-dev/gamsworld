...
* (This version modified to use outward pointing vectors.)
*###############################################################
* Objective: linear
* Constraints: convex quadratic
* Feasible set: convex
...
***************************************************************************
Parameter lx ; lx = sin(pi*60/180)*cos(pi*0/180);
Parameter ly ; ly = sin(pi*60/180)*sin(pi*0/180);
...
Set P_Sidelobes[m,n]; P_Sidelobes[m,n]:=yes${{-(ord(m)-12)*lx -
                                              (ord(n)-12)*ly +
                                                   sqrt(abs(1-sqr(ord(m)-12)-
                                                              sqr(ord(n)-12)))*lz
                      le cosbeamwidth}and (P[m,n])};



Parameter rad ; rad = lz*sqrt((1-cosbeamwidth*cosbeamwidth)/(lx*lx+ly*ly));
set  THETA /l1*l50/;

set P_Sidelobes2[Theta] ;
Parameter PP_Sidelobes2[Theta];
          PP_Sidelobes2[Theta] = (lx*(rad*cos((ord(Theta)-1))-cosbeamwidth)-ly*rad*sin((ord(Theta)-1))/lz+
                                  ly*(rad*cos((ord(Theta)-1))-cosbeamwidth)+lx*rad*sin((ord(Theta)-1))/lz);


Parameter cosine[m,n,i,j] ; cosine[m,n,i,j]=  cos(-2*pi*((ord(m)-12)*(ord(i)-5) + (ord(n)-12)*(ord(j)-5)));
Parameter sine[m,n,i,j]   ; sine[m,n,i,j]  =  sin(-2*pi*((ord(m)-12)*(ord(i)-5) + (ord(n)-12)*(ord(j)-5)));
...
Def_obj.. L_two=e=
      sum {(m,n)$P_Sidelobes[m,n],
        ((sqr(A_real[m,n]) + sqr(A_imag[m,n]))/sqrt(1.01-(sqr(Par_Px[m,n])+sqr(Par_Py[m,n]))))};
...
Solve nb_L2_ using nlp minimazing L_two;
Display L_two.l ;