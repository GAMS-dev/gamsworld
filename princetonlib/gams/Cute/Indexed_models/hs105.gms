...
Eq_1(i)..  a[i] =e= x['j1']/x['j6']*exp(-sqr(y[i]-x['j3'])/(2 *x['j6']*x['j6']));
Eq_2(i)..  b[i] =e= x['j2']/x['j7']*exp(-sqr(y[i]-x['j4'])/(2 *x['j7']*x['j7']));
Eq_3(i)..  c[i] =e= (1 - x['j2'] - x['j1']) / x['j8']
                                   *exp(-sqr(y[i]-x['j5'])/(2 *x['j8']*x['j8']));
C1.. 1 - x['j1'] - x['j2'] =g= 0.0 ;

Def_obj.. obj =e= -sum{i, log((a[i] + b[i] + c[i])/sqrt(2*PI)) };

* From Equation 1 and too are transparent that a and b >0, but We had added this. 
a.lo[i]=0.0000001;
b.lo[i]=0.0000001;
...
*let x.l['j1'] =     0.4128928 ;
*let x.l['j2'] =     0.4033526 ;
*let x.l['j3'] =   131.2613    ;
*let x.l['j4'] =   164.3135    ;
*let x.l['j5'] =   217.4222    ;
*let x.l['j6'] =    12.28018   ;
