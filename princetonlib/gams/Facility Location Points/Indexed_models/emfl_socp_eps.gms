...
Def_s(k,i)..
 s(k,i)  =g=  sqrt(epsilon+sum{j,sqr(x[i,j]-a[k,j] )})    ;

def_t(i,ii)..
 t(i,ii) =g=  sqrt(epsilon+sum{j,sqr(x[i,j]-x[ii,j])})    ;
...
model emfl_socp_eps /all/;

For (mn = 1 to 1,

solve emfl_socp_eps using nlp minimaze sumEucl;
