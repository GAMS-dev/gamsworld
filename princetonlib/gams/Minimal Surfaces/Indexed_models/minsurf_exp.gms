...
t1_bound(x,y)$(Left_x(x) and Left_y(y))..
  exp{(hx*hy/2)*sqrt(1 + sqr((z[x+1,y]  -z[x,y])  /hx)+
                         sqr((z[x,y+1]  -z[x,y])  /hy)) - (t1(x,y))} =l= 1 ;


t2_bound(x,y)$(Left_x(x) and Left_y(y))..
  exp{(hx*hy/2)*sqrt(1+sqr((z[x+1,y+1]-z[x,y+1])/hx)+
                       sqr((z[x+1,y+1]-z[x+1,y])/hy))   - (t2(x,y) )} =l= 1 ;

Def_obj..  area_bnd =e= sum{(x,y)$(Left_x(x) and Left_y(y)),(t1(x,y)+t2(x,y))};
...
Model minsurf_exp /all/ ;

Solve minsurf_exp using nlp minimazing area_bnd ;
