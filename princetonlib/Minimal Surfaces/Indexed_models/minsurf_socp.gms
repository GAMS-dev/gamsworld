...
t1_bound(x,y)$(Left_x(x) and Left_y(y))..
   {(hx*hy/2)*sqrt(1 + sqr((z[x+1,y]  - z[x,y])  /hx)+
                      sqr((z[x,y+1]  - z[x,y])  /hy)) } =l=  (t1(x,y)) ;


t2_bound(x,y)$(Left_x(x) and Left_y(y))..
   {(hx*hy/2)*sqrt(1+sqr((z[x+1,y+1] - z[x,y+1])/hx)+
                    sqr((z[x+1,y+1] - z[x+1,y])/hy))   } =l= (t2(x,y) ) ;
...
Solve ms_socp using nlp minimazing area_bnd ;
