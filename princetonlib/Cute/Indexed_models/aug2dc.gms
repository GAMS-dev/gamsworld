...
Set inside_1_i(i) /i1*i100/;
Set inside_1_j(j) /j1*j100/;
...
Def_obj.. f=e=
        ...
        + sum {i$inside_1_i(i),  sqr(x[i,'j0'] - 1)    }
        + sum {i$inside_1_i(i),  sqr(x[i,'j%ny%'] - 1)  }
        + sum {j$inside_1_j(j),  sqr(y['i0',j] - 1)    }
        + sum {j$inside_1_j(j),  sqr(y['i%nx%',j] - 1)  }

              )
               / 2;

x.lo[i,'%ny1%']$inside_i(i) =1.0;
y.lo['%nx1%',j]$inside_j(j) =1.0;

Model aug2dc /all/;

Solve aug2dc using nlp minimize f;
