...
Def_obj.. f=e=
 sum{(i,j)$inside(i,j),
       ( 0.5*sqr(x[i,j] -x[i,j-1]   ) +   0.5*sqr(x[i,j]-x[i-1,j]      )+
       hp2*power((x[i,j]-x[i,j-1]),4) +hp2*power((x[i,j]-x[i-1,j]),4))} +
 sum{j$inside_j(j),(wght*x['i%p%',j])};
