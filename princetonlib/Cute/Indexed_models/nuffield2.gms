...
conv(i,j)$((inside_i(i)) and (inside_j(j)))..
 (v[i,j+1]-2*v[i,j]+v[i,j-1])/h/h*((v[i+1,j]-2*v[i,j]+v[i-1,j])/h/h) -
...
Addition(i,j)$((inside_i(i)) and (inside_j(j)))..
             initial(i,j)    =e=   (v[i+1,j]-2*v[i,j]+v[i-1,j]);
...
Solve nuffield2 using nlp minimazing integral ;
