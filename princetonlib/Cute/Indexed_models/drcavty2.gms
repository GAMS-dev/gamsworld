...
(RE/4)*(y[i,j+1]-y[i,j-1])*(  y[i-2,j] + y[i-1,j-1] + y[i-1,j+1]- 4*y[i-1,j] -
                            4*y[i+1,j] - y[i+1,j-1] - y[i+1,j+1]-   y[i+2,j])-
(RE/4)*(y[i+1,j]-y[i-1,j])*(  y[i,j-2] + y[i-1,j-1] + y[i+1,j-1]- 4*y[i,j-1]-
                            4*y[i,j+1] - y[i-1,j+1] - y[i+1,j+1]-   y[i,j+2]))};
...
Solve drcavty2 using nlp minimize f;
Solve drcavty2 using nlp minimize f;
Solve drcavty2 using nlp minimize f;
Solve drcavty2 using nlp minimize f;
Solve drcavty2 using nlp minimize f;
