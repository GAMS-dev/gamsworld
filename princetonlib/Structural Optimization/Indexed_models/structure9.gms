...
$Set N  16
$Set M  16
* n => number of elements across x direction
* m => number of elements across y direction
...
Parameter       mu ;      mu = 1.0 ;
Parameter porosity ; porosity = 0.50 ;
...
load[x,y,'d1']${((ord(x)-1) eq 0  ) and (mod((ord(y)-1),kk) eq 0)} = -kk ;
load[x,y,'d1']${((ord(x)-1) eq %n%) and (mod((ord(y)-1),kk) eq 0)} =  kk ;
load[x,y,'d2']${((ord(y)-1) eq 0  ) and (mod((ord(x)-1),kk) eq 0)} = -kk ;
load[x,y,'d2']${((ord(y)-1) eq %m%) and (mod((ord(x)-1),kk) eq 0)} =  kk ;

load['x0','y%m%','d1'] = 0; load['x0','y%m%','d2'] = 0;
load['x%n%','y0','d1'] = 0; load['x%n%','y0','d2'] = 0;
...
Parameter rho(x,y); rho[x,y] = 0.0;
Loop((x,y),
rho[x,y]${sqr(mod((ord(x)-1),kk)-(mod(%n%,kk)-1)/2) +
          sqr(mod((ord(y)-1),kk)-(mod(%m%,kk)-1)/2) ge 43 } = 1.0;
     );
...
compat2(x,y,d)$( (X0(X)) and (Y0(Y)) and (ord(X) gt 1) and (ord(Y) gt 1) )..
         ...
         )*(rho[x-(ord(i)-1),y-(ord(j)-1)]+0.0001)}
         =e= load[x,y,d] ;

Def_obj_3.. complian =e= sum{(x,y,d)$((ord(X) gt 1) and (ord(Y) gt 1) ), load[x,y,d]*w[x,y,d]*length*length } ;
model compl_mod / compat2 ,  Def_obj_3 /;

solve compl_mod  using nlp minimazing complian;
